...
@router.post("/shorten")
def shorten_url(
    request: Request, original_url: str = Form(...), db: Session = Depends(get_db)
):
    url_mapping = url_service.create_short_url(db=db, original_url=original_url)

    base_url = str(request.base_url)
    short_url = f"{base_url}{url_mapping.short_code}"

    return templates.TemplateResponse(
        request, "index.html", {"short_url": short_url, "original_url": original_url}
    )


@router.get("/{short_code}")
def redirect_to_original_url(short_code: str, db: Session = Depends(get_db)):
    url_mapping = url_service.get_original_url(db=db, short_code=short_code)
